...
#define RTTI_ENUM_BEGIN(name)	\
	...
	template<> struct RTTIResolver<render_rtti_ ## name::EnumType_t, true>\
	{\
...
#define RTTI_STRUCT_BEGIN(name)	\
	...
	{\
		...
		typedef render::name ## Dynamic DynamicStructType_t;\
		const RenderRTTIMainType g_mainType = RenderRTTIMainType::name;\
		extern RenderRTTIStructType g_type;\
	}\
	template<> struct RTTIResolver<render_rtti_ ## name::StructType_t, true>\
	{\
		static const RenderRTTITypeBase *GetRTTIType()\
		{\
			return &render_rtti_ ## name::g_type.m_base;\
...
#define RTTI_STRUCT_FIELD(name)	\
	{\
		#name,\
		sizeof(#name) - 1,\
		RTTIResolver<\
			decltype(StructType_t::m_ ## name),\
			RTTIConfigurableTypeCheck<\
				decltype(StructType_t::m_ ## name),\
				decltype(DynamicStructType_t::m_ ## name)\
			>::kIsValid\
		>::GetRTTIType,\
		RTTIConfigurableResolver<decltype(DynamicStructType_t::m_ ## name)>::kIsConfigurable,\
		offsetof(StructType_t, m_ ## name),\
		offsetof(DynamicStructType_t, m_ ## name)\
	},
...
#define RTTI_STRUCT_END	\
	...
	{
		template<class T, bool TTypeCheck>
		struct RTTIResolver
		{
		};

		template<class T, class TOther>
		struct RTTIConfigurableTypeCheck
		{
			static const bool kIsValid = false;
		};

		template<class T>
		struct RTTIConfigurableTypeCheck<T, T>
		{
			static const bool kIsValid = true;
		};

		template<class T>
		struct RTTIConfigurableTypeCheck<T, render::ConfigurableValue<T>>
		{
			static const bool kIsValid = true;
		};
		...
		template<class T>
		struct RTTIConfigurableResolver<rkit::render::ConfigurableValue<T>>
		{
		...
		{
			return static_cast<uint8_t>(static_cast<const render::ConfigurableValue<T> *>(configurablePtr)->m_state);
		}

		template<class T>
		unsigned int EnumHelper<T>::ReadConfigurableValue(const void *configurablePtr)
		{
			return static_cast<unsigned int>(static_cast<const render::ConfigurableValue<T> *>(configurablePtr)->m_u.m_value);
		}

		template<class T>
		const render::ConfigStringIndex_t &EnumHelper<T>::ReadConfigurableName(const void *configurablePtr)
		{
			return static_cast<const render::ConfigurableValue<T> *>(configurablePtr)->m_u.m_configName;
		}

		template<class T>
		void EnumHelper<T>::WriteConfigurableValue(void *configurablePtr, unsigned int value)
		{
			*static_cast<render::ConfigurableValue<T> *>(configurablePtr) = render::ConfigurableValue<T>(static_cast<T>(value));
		}

		template<class T>
		void EnumHelper<T>::WriteConfigurableName(void *configurablePtr, const render::ConfigStringIndex_t &str)
		{
			*static_cast<render::ConfigurableValue<T> *>(configurablePtr) = render::ConfigurableValue<T>(str);
		}

		template<class T>
		void EnumHelper<T>::WriteConfigurableDefault(void *configurablePtr)
		{
			*static_cast<render::ConfigurableValue<T> *>(configurablePtr) = render::ConfigurableValue<T>();
		}
		...
		public:
			...
			{
				return static_cast<double>(static_cast<const render::ConfigurableValue<T> *>(configurablePtr)->m_u.m_value);
			}

			static uint64_t ReadConfigurableValueUIntFunc(const void *configurablePtr)
			{
				return static_cast<uint64_t>(static_cast<const render::ConfigurableValue<T> *>(configurablePtr)->m_u.m_value);
			}

			static int64_t ReadConfigurableValueSIntFunc(const void *configurablePtr)
			{
				return static_cast<int64_t>(static_cast<const render::ConfigurableValue<T> *>(configurablePtr)->m_u.m_value);
			}
		...
		public:
			...
			static uint8_t GetConfigurableState(const void *configurablePtr)
			{
				return static_cast<uint8_t>(static_cast<const render::ConfigurableValue<bool> *>(configurablePtr)->m_state);
			}

			static double ReadConfigurableValueFloatFunc(const void *configurablePtr)
			{
				return (static_cast<const render::ConfigurableValue<bool> *>(configurablePtr)->m_u.m_value) ? 1 : 0;
			}
			...
			static const render::ConfigStringIndex_t &ReadConfigurableName(const void *configurablePtr)
			{
				return static_cast<const render::ConfigurableValue<bool> *>(configurablePtr)->m_u.m_configName;
			}

			static void WriteConfigurableValueFloat(void *configurablePtr, double value)
			{
				*static_cast<render::ConfigurableValue<bool> *>(configurablePtr) = render::ConfigurableValue<bool>(value != 0);
			}
			...
			static void WriteConfigurableName(void *configurablePtr, const render::ConfigStringIndex_t &str)
			{
				*static_cast<render::ConfigurableValue<bool> *>(configurablePtr) = render::ConfigurableValue<bool>(str);
			}

			static void WriteConfigurableDefault(void *configurablePtr)
			{
				*static_cast<render::ConfigurableValue<bool> *>(configurablePtr) = render::ConfigurableValue<bool>();
			}
		};
		...
		template<>
		struct RTTIResolver<bool, true> : public RTTIAutoNumber<bool, RenderRTTINumberBitSize::BitSize1, RenderRTTINumberRepresentation::UnsignedInt>
		{
		};

		template<>
		struct RTTIResolver<float, true> : public RTTIAutoNumber<float, RenderRTTINumberBitSize::BitSize32, RenderRTTINumberRepresentation::Float>
		{
		};

		template<>
		struct RTTIResolver<double, true> : public RTTIAutoNumber<double, RenderRTTINumberBitSize::BitSize64, RenderRTTINumberRepresentation::Float>
		{
		};

		template<>
		struct RTTIResolver<uint8_t, true> : public RTTIAutoNumber<uint8_t, RenderRTTINumberBitSize::BitSize8, RenderRTTINumberRepresentation::UnsignedInt>
		{
		};

		template<>
		struct RTTIResolver<uint16_t, true> : public RTTIAutoNumber<uint16_t, RenderRTTINumberBitSize::BitSize16, RenderRTTINumberRepresentation::UnsignedInt>
		{
		};

		template<>
		struct RTTIResolver<uint32_t, true> : public RTTIAutoNumber<uint32_t, RenderRTTINumberBitSize::BitSize32, RenderRTTINumberRepresentation::UnsignedInt>
		{
		};

		template<>
		struct RTTIResolver<uint64_t, true> : public RTTIAutoNumber<uint64_t, RenderRTTINumberBitSize::BitSize64, RenderRTTINumberRepresentation::UnsignedInt>
		{
		};

		template<>
		struct RTTIResolver<int8_t, true> : public RTTIAutoNumber<int8_t, RenderRTTINumberBitSize::BitSize8, RenderRTTINumberRepresentation::SignedInt>
		{
		};

		template<>
		struct RTTIResolver<int16_t, true> : public RTTIAutoNumber<int16_t, RenderRTTINumberBitSize::BitSize16, RenderRTTINumberRepresentation::SignedInt>
		{
		};

		template<>
		struct RTTIResolver<int32_t, true> : public RTTIAutoNumber<int32_t, RenderRTTINumberBitSize::BitSize32, RenderRTTINumberRepresentation::SignedInt>
		{
		};

		template<>
		struct RTTIResolver<int64_t, true> : public RTTIAutoNumber<int64_t, RenderRTTINumberBitSize::BitSize64, RenderRTTINumberRepresentation::SignedInt>
		{
		};
		...
		RTTI_STRUCT_BEGIN(SamplerDesc)
			...
			RTTI_STRUCT_FIELD(haveMaxLod)
			RTTI_STRUCT_FIELD(anisotropy)
			RTTI_STRUCT_FIELD(isCompare)
	...
	*/
	...
	{
		return reinterpret_cast<const RenderRTTIStructType *>(render_rtti::RTTIResolver<render::SamplerDesc, true>::GetRTTIType());
	}
}
