...
namespace rkit
{
	...
	struct DefaultPathTraits
	{
		...
		static PathValidationResult ValidateComponent(const BaseStringSliceView<Char_t> &span, bool isAbsolute, bool isFirst, bool isLast);
		static void MakeComponentValid(const Span<Char_t> &span, bool isAbsolute, bool isFirst, bool isLast);
...
#error "Unknown platform FS traits"
	...
	public:
		...
		bool operator==(const BasePathView<TIsAbsolute, TPathTraits> &path) const;
		bool operator!=(const BasePathView<TIsAbsolute, TPathTraits> &path) const;
	...
	public:
		...
		Result Set(const BasePathView<TIsAbsolute, TPathTraits> &path);
...
namespace rkit
{
	...
	template<uint32_t TTraitFlags>
	inline PathValidationResult DefaultPathTraits<TTraitFlags>::ValidateComponent(const BaseStringSliceView<Char_t> &span, bool isAbsolute, bool isFirst, bool isLast)
	{
		...
		if (!GetDrivers().m_utilitiesDriver->DefaultIsPathComponentValid(span, isFirst, isLast, (TTraitFlags & PathTraitFlags::kAllowWildcards) != 0))
	...
	template<uint32_t TTraitFlags>
	inline void DefaultPathTraits<TTraitFlags>::MakeComponentValid(const Span<Char_t> &span, bool isAbsolute, bool isFirst, bool isLast)
	{
		if (TTraitFlags & PathTraitFlags::kIsCaseInsensitive)
		{
			for (Char_t &ch : span)
	...
	template<bool TIsAbsolute, class TPathTraits>
	BasePathView<TIsAbsolute, TPathTraits>::BasePathView(const BaseStringView<Char_t> &view)
		: m_view(view)
	{
	}
	...
	template<bool TIsAbsolute, class TPathTraits>
	const typename BasePathView<TIsAbsolute, TPathTraits>::Char_t *BasePathView<TIsAbsolute, TPathTraits>::GetChars() const
	{
		return m_view.GetChars();
	}

	template<bool TIsAbsolute, class TPathTraits>
	size_t BasePathView<TIsAbsolute, TPathTraits>::Length() const
	{
		return m_view.Length();
	}

	template<bool TIsAbsolute, class TPathTraits>
	typename BasePathView<TIsAbsolute, TPathTraits>::Component_t BasePathView<TIsAbsolute, TPathTraits>::operator[](size_t index) const
	{
		size_t scanPos = 0;
		while (index < 0)
		{
	...
	template<bool TIsAbsolute, class TPathTraits>
	size_t BasePathView<TIsAbsolute, TPathTraits>::NumComponents() const
	{
		if (m_view.Length() == 0)
			return 0;
	...
	template<bool TIsAbsolute, class TPathTraits>
	bool BasePathView<TIsAbsolute, TPathTraits>::operator==(const BasePathView<TIsAbsolute, TPathTraits> &other) const
	{
	...
	Result BasePath<TIsAbsolute, TPathTraits>::AppendComponent(const BaseStringSliceView<Char_t> &str)
	{
		...
		chars[str.Length()] = kDefaultDelimiter;
		CopySpanNonOverlapping(chars.SubSpan(m_path.Length() + 1, chars.Count() - m_path.Length() - 1), str.ToSpan());
	...
	template<bool TIsAbsolute, class TPathTraits>
	Result BasePath<TIsAbsolute, TPathTraits>::Set(const BasePathView<TIsAbsolute, TPathTraits> &view)
	{
		return m_path.Set(view.ToStringView());
	}
	...
	PathValidationResult BasePath<TIsAbsolute, TPathTraits>::Validate(const BaseStringSliceView<Char_t> &str)
	{
		...
		for (size_t scanPos = 0; scanPos <= str.Length(); scanPos++)
		{
			if (scanPos == str.Length() || TPathTraits::IsDelimiter(str[scanPos]))
			{
				...
				const bool isLast = (scanPos == str.Length());
				...
				const PathValidationResult result = TPathTraits::ValidateComponent(slice, TIsAbsolute, isFirst, isLast);
...
namespace rkit
{
	...
	inline PathValidationResult OSPathTraits::ValidateComponent(const BaseStringSliceView<Char_t> &span, bool isAbsolute, bool isFirst, bool isLast)
	{
		if (GetDrivers().m_utilitiesDriver->IsPathComponentValidOnWindows(span, isAbsolute, isFirst, isLast, false))
			return PathValidationResult::kValid;

		return PathValidationResult::kInvalid;
	}

	inline void OSPathTraits::MakeComponentValid(const Span<Char_t> &span, bool isAbsolute, bool isFirst, bool isLast)
	{
	}
};
