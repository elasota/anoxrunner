...
#if RKIT_RESULT_BEHAVIOR == RKIT_RESULT_BEHAVIOR_FATAL
#define RKIT_IMPLEMENT_FATAL_ERROR_FUNC	\
	void ::rkit::priv::RaiseFatalError(::rkit::PackedResultAndExtCode resultAndExtCode)\
	{\
		if (::rkit::ISystemDriver *sysDriver = ::rkit::GetDrivers().m_systemDriver.Get()) \
			sysDriver->FatalError(resultAndExtCode); \
		std::abort(); \
	}
...
#define RKIT_IMPLEMENT_LINKER_BOUNDARY_PER_MODULE_FUNCTIONS	\
	RKIT_IMPLEMENT_RESULT_FIRST_CHANCE_RESULT_FAILURE\
	RKIT_IMPLEMENT_ASSERTION_CHECK_FUNC\
	RKIT_IMPLEMENT_FATAL_ERROR_FUNC


#if RKIT_MODULE_LINKER_TYPE == RKIT_MODULE_LINKER_TYPE_DLL

#define RKIT_IMPLEMENT_PER_MODULE_FUNCTIONS RKIT_IMPLEMENT_LINKER_BOUNDARY_PER_MODULE_FUNCTIONS
